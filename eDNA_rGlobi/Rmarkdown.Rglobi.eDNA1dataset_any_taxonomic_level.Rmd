...
```{r}
taxonomytable<-read.delim("./data/LA_river_X12S_ASV_sum_by_taxonomy_70_dc_ed_min5_LA_river_X12S_ASV_sum_by_taxonomy_70_dc_ed_min5.txt")
...
```{r}
# TODO: Since you want other people to reuse your code, asking users to type 
# family four times is not the most user friendly interface. People should just 
# type family once, and everything else should work.
...
# TODO: Since you only want the taxa names, save the column values  
# as a vector instead of changing table. You can create a new object called
# taxa_list, and assign the unique, not NA values from a particular column.
...
# TODO: You can use dimnames to add the row and column names when creating 
# the matrix.
BinaryMatrix <- matrix(NA, number_eDNA_spp, number_eDNA_spp)
#add species names to the columns/rows 

# TODO: Don't need [1:number_eDNA_spp] when assigning col and row names 
# when the rows and columns have the same number of items as the vector. 
# TODO: You can use taxa_list instead of referring to the table.
...
# TODO: Simplify this function. Replace all the code you have with one line.
# return(nrow(rGlobiResult) > 0)
...
how_they_interact <-  function(rGlobiResult)
{
    ...
    # TODO: return paste0(...) instead of assigning it to an object since 
    # you are not using the newly assigned object
...
for(i in 1:number_eDNA_spp)
{
 ...
 interaction_result <-get_interactions_by_taxa(source_species, target_species, showfield = c("source_taxon_name", "interaction_type", "target_taxon_name"), otherkeys = list("limit"=1)) 
    ...
    # TODO: 'TRUE==' is unneccesary since do_they_interact returns a boolean. 
...
```{r}
# TODO: You can use dimnames to add the row and column names when creating the 
# matrix.
